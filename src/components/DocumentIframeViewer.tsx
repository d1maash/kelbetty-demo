...
import { FileText, Download, Loader2, AlertCircle, Settings, Type } from 'lucide-react'

interface DocumentIframeViewerProps {
    document: {
        id: string
        title: string
        html: string
...
export default function DocumentIframeViewer({
    document,
    onSave,
    onAcceptChanges,
    onRejectChanges,
    ...
    const createIframeContent = (html: string, css?: string) => {
        // Удаляем любые <script> для предотвращения попытки выполнения в sandbox
        const safeHtml = html.replace(/<script[\s\S]*?<\/script>/gi, '')
        // Анализируем используемые шрифты
        const fontAnalysis = analyzeFonts(safeHtml)
...
<head>
    ...
    <!-- CSP: полностью запрещаем скрипты внутри iframe -->
    <meta http-equiv="Content-Security-Policy" content="default-src 'self' data:; img-src 'self' data: blob:; style-src 'unsafe-inline' 'self' data:; font-src 'self' data:; script-src 'none'">
    <title>${document.title}</title>
    
    <!-- КРИТИЧНО: Полная изоляция от внешних стилей -->
    <style>
        /* Сброс всех стилей для полной изоляции */
        html, body {
            all: initial;
        ...
        *[style] {
        ...
        /* Подключение шрифтов */
        ${fontAnalysis.fontFaces}
        
        /* Внешний CSS от LibreOffice (если есть) */
        ${css || ''}
...
<body>
    ...
    // Анализ шрифтов в HTML
    function analyzeFonts(html: string): { fontFaces: string; warnings: string[] } {
        ...
        // Создаем @font-face для найденных шрифтов
    ...
    useEffect(() => {
        ...
        try {
            ...
            const content = createIframeContent(document.html, document.css)
    ...
    }, [document.html, document.css, document.title])
            ...
            {fontWarnings.length > 0 && (
                <div className="border-b border-yellow-200 bg-yellow-50 p-3">
                    <div className="flex items-center gap-2 text-yellow-800">
                        ...
                        <span className="text-sm font-medium">Предупреждения о шрифтах:</span>
                    </div>
                    <ul className="text-sm text-yellow-700 mt-1">
                        {fontWarnings.map((warning, index) => (
