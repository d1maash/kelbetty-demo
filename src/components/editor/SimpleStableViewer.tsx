...
interface SimpleStableViewerProps {
    document: {
        id: string
        title: string
        html: string
...
export default function SimpleStableViewer({ document, onSave }: SimpleStableViewerProps) {
    const editorRef = useRef<HTMLDivElement>(null)
    const [isEditing, setIsEditing] = useState(false)
    const [hasChanges, setHasChanges] = useState(false)

    useEffect(() => {
        console.log('SimpleStableViewer: Загружаем документ:', document.title)

        if (editorRef.current && document.html) {
            // Простая загрузка без манипуляций с DOM
            editorRef.current.innerHTML = document.html
            console.log('SimpleStableViewer: Документ загружен')
        }
        ...
        <div className="h-full flex flex-col bg-white">
            ...
            <div className="border-t border-slate-200 p-3 bg-slate-50">
                <div className="flex items-center justify-between text-xs text-slate-500">
                    <p>
                        💡 {isEditing ? 'Режим редактирования' : 'Режим просмотра'} • Простой стабильный
                    </p>
                    <p>
